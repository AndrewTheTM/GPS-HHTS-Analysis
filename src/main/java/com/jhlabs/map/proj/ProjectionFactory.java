...
public class ProjectionFactory {
    ...
    private final static double SEC_TO_RAD = Math.PI/180/3600;
...
//		projection.setProjectionLatitude2Degrees( 0 );
        ...
        s = (String) params.get("towgs84");
        if (s != null) {
          String[] tokens = s.split(",");
          if( tokens.length != 3 && tokens.length != 7) {
            throw new ProjectionException("expected towgs84 parameter to have 3 or 7 values but found " + tokens.length +": " + s);
          }
          double[] wgsParams = new double[tokens.length];
          for( int i=0; i<tokens.length; i++){
            wgsParams[i] = Double.parseDouble(tokens[i]);
          }
          if( wgsParams.length == 7){
            /* transform from arc seconds to radians */
            wgsParams[3] *= SEC_TO_RAD;
            wgsParams[4] *= SEC_TO_RAD;
            wgsParams[5] *= SEC_TO_RAD;
            /* transform from parts per million to scaling factor */
            wgsParams[6] = 
                (wgsParams[6]/1000000.0) + 1;
          }
          projection.ellipsoid.datumParams = wgsParams;
        }
...
//		register( "laea", Projection.class, "Lambert Azimuthal Equal Area" );
            ...
            register("latlong", LatLongProjection.class);
            register("longlat", LatLongProjection.class);
